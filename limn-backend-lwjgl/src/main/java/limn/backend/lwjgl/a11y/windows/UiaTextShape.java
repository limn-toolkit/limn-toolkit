...
/**
 * The {@code TEXT} shape's half of the Windows bridge: an editable string, and a value's spoken
 * form: Value, its setter and its read-only bit (a spinner's "07:30" and a combo's item vend it
 * too, because a reader prefers a string to a number where both are offered).
 *
        ...
        switch (patternId) {
            case UiaIds.VALUE_PATTERN -> {
                slots.put("SetValue", (UiaCom.PP) (self, text) -> {
                    // Semantics 5: the whole text of a text facet is SET_TEXT; the
