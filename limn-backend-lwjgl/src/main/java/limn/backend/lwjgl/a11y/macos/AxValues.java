 ...
 * otherwise.
 *
 * <p><b>A value with no number answers no number</b>: a date segment nobody has typed into
 * publishes {@link limn.accessibility.ValueFacet#empty()} with its minimum in the number, because
 * two other platforms must answer a number there, and a word in its text. This platform has no
 * attribute that demands a number — {@code AXValue} is an object — so an empty value answers its
 * word, and with no text at all nothing, and never the minimum as if it had been typed. A change of
 * the text alone is a {@code VALUE_CHANGED} in the model, which this bridge posts as a value change
 * like any other.
 */
