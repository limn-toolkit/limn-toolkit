 ...
 * longer holds answers {@code UIA_E_ELEMENTNOTAVAILABLE}, which is the answer promised to a client
...
final class UiaProvider {
    ...
    interface Context {
         ...
         * none. A bridge holding several windows also answers true for the node another window's
         * cursor resolved into this one.
         *
         ...
         * that window's fragment pointer, from that window's own provider.
         *
         ...
         * element-valued property whose target is not in this window answers with: the opener of a
         * native popup carries {@code CONTROLLER_FOR} naming the popup's root, which the platform
         * put in a window of its own, and node identifiers have been process-wide since 2026-09-14,
         * so such a target is a real node and not a dangling number.
         *
         ...
         * native popup whose opener's cursor resolved into it. What this window's own
         ...
         * {@code UiaClientsAreListening()} is true on an ordinary desktop with no reader. Called on
         * an RPC thread.
         *
         ...
         * because a client's thread must not wait on the user-interface thread, and because the
         * widget performs it through its own private path with its own guards. What a client is
         * told is that the request was understood.
         *
    ...
    /**
     * <p>{@code FOCUS}, posted only where the node publishes it now, and refused synchronously
     * otherwise the way every pattern verb is ({@link UiaPatternProviders#refusal}, whose javadoc
     * says which platform providers were read answering {@code SetFocus} not-enabled first, and
     * which do not). A focusable widget's FOCUS is the walk's free verb; an item publishes it where
     * moving the cursor does not select. Until 2026-09-15 it was posted for any node, the root and
     * a disabled button included.
     *
     ...
     * {@code ElementNotEnabledException} for a disabled window and
     * {@code InvalidOperationException} for an element that cannot take the keyboard, while WPF's
     * {@code ElementProxy.SetFocus} checks no enabled bit at all
     * (readings/windows-dump-uia-focus-and-scroll-item.txt). This bridge answers as it answers
     * every other verb; {@link UiaPatternProviders#refusal} carries the reasoning.
     */
    ...
    private static int propertyValueAnswering(long nodeId, int propertyId, long out,
        ...
        // window's own provider (2026-09-15; ADR 039 §1.11's 2026-09-14 amendment): since
    ...
    /**
     * One element-valued relation target, from whichever window holds it.
     *
     ...
     * {@code scripts/a11y/windows/dump-uia-typelib.ps1 -AllMembers}). Answering the opener as the
     * popup's own {@code ControllerFor} instead would say the popup controls the field that opened
     * it, which is the relation backwards. The rule for relations across windows asked for a
     * ControllerFor or PopupFor element from the other HWND's provider; only the first half is
     * answerable on this platform, and the rule was amended on that point on 2026-09-15. Pinned by
