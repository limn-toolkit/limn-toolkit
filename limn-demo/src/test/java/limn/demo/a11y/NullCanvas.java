...
/**
 * A canvas that draws nothing, so a whole demo scene can run its frames with no GL. Only the
 * save/restore depth is real: the toolkit balances it and complains when it does not. Text is
 * measured by {@link HeadlessWindow#RULER}, the same fixed ruler every widget lays out against,
 * so a transcript taken here is the same on every machine.
 *
 * <p>A copy of the theme editor's, and deliberately a copy: a shared test-support module is the
 * only alternative, and every module would then depend on it.
 */
final class NullCanvas implements Canvas {
    ...
    @Override public TextMetrics measureText(String text, Font font) {
        return HeadlessWindow.RULER.measure(text, font);
    }
