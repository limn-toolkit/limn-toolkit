// What CLDR says about a locale, for the two tables this toolkit writes out by hand: which
// digits a language writes numbers in, and which direction its script runs.
//
// The sibling of dump-cldr-plurals.mjs and the same argument: Node's Intl is CLDR through ICU,
// so it is the source limn.i18n.NumberingSystem and limn.scene.LayoutDirection are supposed to
// agree with, and a table nobody checks is a table that drifts. Run it and write its output over
...
//
// CldrLocaleFactsTest compares both tables against it. Nothing in the build runs Node.
//
