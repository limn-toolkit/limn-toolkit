...
> *2026-09-03.* `setControlSizeHost` is now `setInheritanceHost`: the out-of-tree link this document designs carries every inherited axis, not the size alone, once ADR 032 gave it a direction and ADR 035 a locale, and a name that said "control size" promised less than the link does. The old name stayed as a deprecated delegate and was removed in 27b3a4e, because nobody had released against it. The text below keeps the name it was written with; every `setControlSizeHost` in it reads as `setInheritanceHost` today, with the same ordering rules.
...
The parent walk cannot cross a `Scene` boundary or an overlay, so **five** sites need an explicit host link (revision 1 listed four and missed the in-scene menu overlay, which is the path every headless test takes). *The link is `setInheritanceHost` since the rename noted in §1; the sites and the ordering below are unchanged:*
