...
**In force.**
- After layout the UI thread publishes an immutable whole-window `AccessibleTree`, and only when a
  field differs; bridges read it on any thread with no lock and no hop (§1.1, §3.5, §5.3).
- Roles, states and facets are closed lists; the setter refuses a state a facet or the walk owns,
  and a name carries its provenance and the node's locale (§1.2, §1.7, §1.12; amended 2026-09-14).
- Identity is (owner, local key) over the widget tree, a parent's key given before the child
  describes itself, tagged per scene: it survives frames and names a node in any window (§1.3).
- A widget describes itself and acts through six protected hooks on `Widget`; an application names,
  re-roles or ignores one through public setters and speaks through `Scene#announce` (§1.5, §1.6).
- A node accepts only the verbs it publishes and the setters its facets imply, while `ENABLED` and
  `VISIBLE`; `Host#perform` posts, and the UI thread re-checks, reveals and performs (§1.5, §1.9).
- Events are the difference of two snapshots: the cursor is the tree's, selection is one event per
  container, structure one per surviving parent, and a reserved tail survives a collapse (§1.10).
  The bridge subscribes to nothing, ADR 040's channel included (§9.3).
- Windows raises from a drain thread; a macOS callback is the UI thread and posts at `frameEnded`;
  Linux is one application, embedded for the process's life once enabled (§3, §5.3, decision 67).
- An in-scene popup is a `MODAL` subtree, a native one its own window; outside the input layer
  nothing is `ENABLED` or `FOCUSABLE`, and relations cross windows (§1.11, §1.13).
- A bridge diverges only where §4.2 declares it (a Linux menu title, a macOS row's focus write, a
  stale VoiceOver select), and every constant is read off a guest (§12.3).

**Changed after acceptance.**
- Four hooks became six: identity is answered first, and a container may claim a verb on a widget
  child; "no third kind" is withdrawn for a widget under a synthetic row (§1.3, §1.5, §7.1).
- The cursor left each `SelectionFacet` for the tree, and per-node selection and structure events
  became per container and per parent (§1.10; amended 2026-09-14).
- §1.9's showing gate became a visibility gate that reveals, with `VISIBLE` beside `ENABLED` as the
  operable bit; publishing a disabled node `READ_ONLY` was withdrawn (decision 66, fix round 2e).
...
**Open.**
- The owner's: `SELECTABLE` on a `Table` or `Tree` row in `NONE` (§1.2, amended 2026-09-16), an
  absent macOS selector skipped (§12.3), two Windows choices (appendix A §2.1, §2.4).
- macOS: VoiceOver's cursor not following the application's in an outline or table (§4.2);
  `AXModal`, text ranges, min/max value not installed; a run with VoiceOver off (appendix A §2.2).
- Windows: a small Scroll step jumped a table to 95.65 %, then nothing moved; a scrolled branch's
  rows are heard a level high (appendix A §2.1); `ControllerFor` not read live (§1.11).
- Unheard: a busy sorted header, a collapse's re-announcement, `ChildrenReordered` (appendix A
  §2.1, §2.4); Orca on `sort` alone, Orca 46.1 over a popup (appendix A §2.3, appendix B §12.2).
- Linux menu titles: the role, the verb names and the nesting under the title (§4.2). Owed: the
  owner's read-aloud of `table.txt` (§12.1).
- Risks kept: a client attaching during a Win32 file dialog, the macOS pump cliff's cause, abstract
  D-Bus sockets, one-architecture ABI facts (appendix B §13). Undecided: §15. Left out: §11.

**Where to read.** Status block; §1.1 to §1.13, each to its last dated amendment; §3 for threads;
§4.2 for divergences; appendix A for what each bridge serves, appendix B for the lab and the risks.
...
### 2.1 to 2.4: each platform, and the events side by side

What each bridge serves on Windows (§2.1), macOS (§2.2) and Linux (§2.3), interface by interface,
and the events of the three side by side (§2.4), with every amendment the live runs brought, are in
[appendix A](039-appendix-a-the-three-platforms-interface-by-interface.md).
...
What only a platform client or a screen reader can prove, how the lab runs it, and what each
round found, are in [appendix B](039-appendix-b-the-lab-and-the-risks.md).
...
The risks this record took and the edges it leaves open, with how each stands, are in
[appendix B](039-appendix-b-the-lab-and-the-risks.md). The summary at the top lists the ones still open.
