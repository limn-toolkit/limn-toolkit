...
платформы и не может знать, на какую машину попадёт: это отдельный артефакт, а не классификатор, и
он называет все шесть за вас. Указать несколько классификаторов тоже ничто не мешает: дистрибутиву
на две цели нужны две.
