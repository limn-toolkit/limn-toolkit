...
- **Справа налево — это строка, которую вы пишете, а не то, что подразумевает язык.** Арабский,
  иврит, деванагари и тайский формируются всюду, где рисуется текст, а
  `scene.setLayoutDirection(LayoutDirection.RTL)` отзеркаливает весь интерфейс — ряды, отступы,
  выравнивание, полосы прокрутки, всплывающие меню, клавиши со стрелками — как ось, которую любое
  поддерево может переопределить. Тулкит никогда не выводит это направление из локали, потому что
  окно на иврите обычно содержит остров слева направо; поэтому эти два свойства объявляются
  отдельно. Вертикальное письмо (монгольское, вертикальная форма CJK) не поддерживается.
- **Программы чтения с экрана работают, и три вещи, которых они пока не могут.** Windows, macOS и
  Linux получают каждая свой родной API доступности — UI Automation, NSAccessibility, AT-SPI2, — так
  что NVDA, VoiceOver и Orca читают окно Limn на языке интерфейса. Текст читается и записывается
  целиком, поэтому просмотр по символам и наведение брайлевского курсора ограничены; список
  публикует только реализованные строки; а системные настройки доступности — высокая контрастность,
  уменьшение движения, системный масштаб текста — не передаются вовсе.
