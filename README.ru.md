...
**Сделан открыто, с помощью Claude Code:** код написал Claude, а направляли и проверяли его люди.
То, чего стоила разработка, уже оплачено, так что платить за неё снова вам не нужно. [Как делается
Limn](#как-делается-limn)
...
JBang один раз определяет новейший релиз и сохраняет его: чтобы получить более новый, добавьте `--fresh`, например `jbang --fresh demo@limn-toolkit/limn-toolkit`.
...
**Таблицы, деревья и даты уже готовы.** Таблица — это типизированные колонки над списком, которым
владеет ваше приложение, отсортированным тулкитом или вашим сервером, с подвалом, который
суммирует; дерево — это структура над потомками, которых вы предоставляете, и строка может
пообещать потомков раньше, чем сумеет их назвать; поле даты вводит дату ISO в календаре и формате
самого читателя, а выбор даты открывает за ним сетку месяца. Программы чтения с экрана на трёх
платформах читают все три.

<p align="center">
  <picture>
    <source media="(prefers-color-scheme: dark)" srcset="media/readme/showcase-data-dark.webp">
    <img src="media/readme/showcase-data-light.webp" alt="Таблица, дерево, поле даты и выбор даты в одном окне Limn" width="900">
  </picture>
</p>
...
подстановка шрифтов выбирает начертание для каждого отрезка одной письменности, поэтому латиница, греческий, кириллица и CJK смешиваются
...
**И всё это читается вслух, на тех же языках.** Окно публикует себя в UI Automation на Windows, в
NSAccessibility на macOS и в AT-SPI2 на Linux, так что NVDA, VoiceOver и Orca описывают интерфейс
Limn, а вашему приложению не нужно ни строки платформенного кода. Имена — те же `I18nString`, что
интерфейс рисует, а собственное слово для каждой роли тулкит несёт на всех языках, которые
поставляет, — потому что две платформы из трёх его не произносят.
...
`limn-theme-editor` — программа, которая её пишет. Запустите её:

```bash
jbang --main limn.themeeditor.ThemeEditorApp io.github.limn-toolkit:limn-theme-editor:x.y.z
```

Тот же флаг macOS, что и выше. То, что она сохраняет, — файл `.limntheme`, обычные данные, которые
ваше приложение загружает через `ThemeFormat` из `limn-toolkit`. Сам редактор никогда не становится
зависимостью вашего приложения.
...
| `limn-test` | тестирование интерфейса без экрана: драйвер, который щёлкает, печатает и нажимает клавиши в сцене, среда выполнения и бэкенд без дисплея и контракты доступности, которым подчиняются виджеты |
...
Код Limn пишет Claude, модель Anthropic, в Claude Code, а направляют и проверяют работу люди. Мы
говорим об этом сразу, потому что вы захотели бы это знать.

Направлять значит, что каждое проектное решение записано в [`docs/adr`](docs/adr), обосновано и
принято до того, как написан код. Проверять значит, что каждое изменение проходит набор тестов,
который больше самого кода; что доступность не предполагалась, а была прослушана: NVDA, VoiceOver и
Orca читали настоящие окна в Windows, macOS и Linux; и что код прошёл повторные аудиты корректности,
производительности и дизайна API.

Путь сюда занял тысячи обращений к модели и миллионы сгенерированных токенов. По модели оценки
COCOMO II более ста тысяч строк кода, не считая тестов, это работа команды примерно из двадцати
человек на срок более двух лет. Эта работа сделана. Вы можете строить на ней, а не повторять её.
...
Работа над самим тулкитом начинается с [`docs/design/README.md`](docs/design/README.md): там
сказано, что относится к Javadoc, что к ADR и что к заметке о дизайне, и собран указатель заметок
