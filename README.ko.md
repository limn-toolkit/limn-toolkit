...
- **오른쪽에서 왼쪽은 직접 쓰는 한 줄이지, 언어가 암시하는 것이 아닙니다.** 아랍 문자, 히브리 문자,
  데바나가리, 타이 문자는 텍스트가 그려지는 곳이라면 어디서나 모양을 갖추고,
  `scene.setLayoutDirection(LayoutDirection.RTL)`은 인터페이스 전체를 — 행, 안쪽 여백, 정렬, 스크롤
  막대, 팝업, 화살표 키 — 각 하위 트리가 재정의할 수 있는 축으로 좌우로 뒤집습니다. 툴킷은 그 방향을
  로캘에서 추론하지 않습니다. 히브리어 창에는 보통 왼쪽에서 오른쪽으로 읽는 섬이 들어 있기 때문에,
  둘은 따로 선언합니다. 세로쓰기(몽골 문자, CJK의 세로 형태)는 지원하지 않습니다.
- **화면 낭독기가 동작하며, 아직 안 되는 것이 셋 있습니다.** Windows와 macOS와 Linux는 각각 자기
  접근성 API — UI Automation, NSAccessibility, AT-SPI2 — 를 받고, NVDA와 VoiceOver와 Orca가 Limn
  창을 인터페이스의 언어로 읽습니다. 텍스트는 통째로 읽고 쓰기 때문에 글자 단위 검토와 점자 커서
  라우팅은 제한됩니다. 목록은 실체화된 행만 공개합니다. 그리고 고대비, 동작 줄이기, 시스템 글자
  크기 같은 운영체제의 접근성 설정은 전혀 노출하지 않습니다.
