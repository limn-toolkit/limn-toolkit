...
- **复杂文字到处都会正确绘制，但没有任何东西是镜像的。** 阿拉伯文、希伯来文、天城文与泰文在任何绘制文字的地方都会连写、会重排，标记也落在该在的位置——`Label`、`TextField`、`TextArea` 里如此，它们之外的每个按钮、标签页、菜单项与占位文字也是如此，`ar` 与 `he` 的翻译照常发布。从右向左的语言拿不到的是版面：内边距、对齐、滚动条在哪一侧、弹出菜单从哪侧展开、文本框之外按方向键会往哪边走，无论什么语言都从左到右。
