...
**ユーザーの言語。** テキストは描画に使うのと同じ送り幅で測られ、フォントフォールバックは 1 文字ずつ働きます。だからラテン、ギリシャ、キリル、CJK が 1 つの文字列に混ざっても書体を選ぶ必要がありません——CJK と絵文字の書体は自分で選んで加える依存関係 1 つ（`limn-fonts-all`）に載っていて、残りはバックエンドに付いてきます。入力メソッドは入力欄の中で変換し、編集は書記素クラスタ単位で動きます。
...
- **右から左は、あなたが書く 1 行であって、言語から暗黙に決まるものではありません。** アラビア文字、ヘブライ文字、デーヴァナーガリー、タイ文字は、テキストが描かれるところならどこでも整形され、`scene.setLayoutDirection(LayoutDirection.RTL)` がインターフェース全体——行、余白、揃え、スクロールバー、ポップアップ、矢印キー——を、各サブツリーが上書きできる軸として鏡像化します。ツールキットがこの向きをロケールから推測することは決してありません。ヘブライ語のウィンドウにはたいてい左から右に読む島があるので、この 2 つは別々に宣言します。縦書き（モンゴル文字、CJK の縦書き）には対応していません。
- **スクリーンリーダーは動きます。ただし、まだできないことが三つあります。** Windows・macOS・Linux はそれぞれのネイティブなアクセシビリティ API — UI Automation、NSAccessibility、AT-SPI2 — に載っていて、NVDA・VoiceOver・Orca が Limn のウィンドウを、そのインターフェイスの言語で読み上げます。テキストは丸ごと読み書きするので、文字単位のレビューと点字カーソルのルーティングは制限されます。リストは実体化した行だけを公開します。そして高コントラスト、視差効果の軽減、システムの文字サイズといった OS のアクセシビリティ設定は一切公開していません。
