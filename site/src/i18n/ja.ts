...
export const ja: Catalog = {
  ...
  "home.everyone.eyebrow": "すべての人へ",
  "home.everyone.heading": "あらゆる言語と、あらゆるスクリーンリーダー",

  "home.languages.heading": "ユーザーの言語で",
  "home.languages.body":
    "ラテン文字・ギリシャ文字・キリル文字・CJK がひとつの文字列に混在し、フォントは同じ文字体系の連なりごとに選ばれます。アラビア文字、ヘブライ文字、デーヴァナーガリー、タイ文字は正しく整形され（連結形、合字、結合記号）、右から左へ書く言語ではウィンドウ全体が鏡像になります：行、余白、スクロールバー、矢印キー。入力メソッドはフィールド内で変換し、編集は書記素単位で進みます。",
  "home.languages.alt":
  ...
  "home.accessibility.body":
    "NVDA・VoiceOver・Orca は各プラットフォーム自身のアクセシビリティ API を通して Limn のウィンドウを読み上げます。アプリケーションにプラットフォーム固有のコードは不要です。各コントロールは、インターフェースの表示言語で、ロール・名前・状態を自分で伝えます。コントロールに名前を付けるのは呼び出しひとつです。",
  ...
  "showcase.description":
    "ツールキットが描画した画面全体。デモアプリケーション、右から左に鏡像化した同じウィンドウ、3D ビューポート、フォーム、レイアウトしたウィンドウ。",
  "showcase.rtl.heading": "同じウィンドウを、右から左に",
  "showcase.rtl.body":
    "ar の翻訳で方向軸を反転させたデモアプリケーション。メニューバーもタブも入力欄もスクロールバーもパフォーマンスフッターも右から読み、その中のテキストはアラビア文字として正しく結合・並べ替えされます。画面を書き直してはいません——方向はコントロールサイズと同じように設定する、継承される一つの軸です。",
  ...
  "showcase.editor.heading": "ファイルを書き出すプログラムとしてのテーマエディタ",
  "showcase.editor.body":
    "パレットはデータなので、それを編集することは一つの画面になります。そしてこれは、あなたのアプリケーションが抱える依存関係ではなく、起動して使うプログラムです。保存されるのはテーマファイルで、アプリケーションは limn-toolkit だけで ThemeFormat を使って読み込みます。角の丸みのスライダーを動かせば、同じフレームのうちに窓が着替えます。画面の中のすべての入力欄、ボタン、色見本がまとめて変わります。隣の判定は、それぞれの色がのりうるすべての面に対して測られるので、コントラストの足りないパレットは目に見えて落ちます。",
