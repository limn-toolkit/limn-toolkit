...
export const zhHant: Catalog = {
  ...
  "home.accessibility.body":
    "NVDA、VoiceOver 和 Orca 透過各平台自己的無障礙 API 讀取 Limn 視窗，你的應用程式無需任何平台程式碼。每個控制項都以介面所用的語言，自己說出角色、名稱和狀態。為控制項命名只需一次呼叫。",
  ...
  "showcase.description":
    "由工具組繪製的完整畫面：示範應用程式、由右至左鏡像的同一個視窗、3D 視埠、一個表單與一個完成版面的視窗。",
  "showcase.rtl.heading": "同一個視窗，由右至左",
  "showcase.rtl.body":
    "在 ar 翻譯下把方向軸反轉的示範應用程式：選單列、分頁、欄位、捲軸與效能頁尾都從右側讀起，其中的文字按阿拉伯文應有的方式連寫與重排。沒有重寫任何畫面——方向是一條繼承的軸，像控制項尺寸一樣設定。",
  ...
  "showcase.editor.heading": "主題編輯器，一個寫出檔案的程式",
  "showcase.editor.body":
    "配色就是資料，所以編輯它本身就是一個介面。而這個介面是一個你執行的程式，不是你的應用程式要背上的相依項。它儲存一個主題檔案，你的應用程式只靠 limn-toolkit，用 ThemeFormat 讀回去。拖動圓角滑桿，視窗就在同一格裡換裝：畫面裡的每個輸入框、按鈕與色塊。旁邊的報告會把每種墨色對著它可能落下的每一種表面來量，所以對比度不過關的配色會當場露餡。",
