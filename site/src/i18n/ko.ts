...
export const ko: Catalog = {
  ...
  "consent.title": "측정을 허용하지 않으면 쿠키를 쓰지 않습니다",
  "consent.body":
  ...
  "consent.analyticsBody":
    "googletagmanager.com에서 불러오는 Google Analytics입니다. 자체 쿠키를 설정하고, 어떤 페이지가 읽히는지 프로젝트에 알려 줍니다. 차단된 상태로 배포되며, 여기서 허용해야만 실행됩니다.",
  ...
  "home.languages.body":
    "텍스트는 그릴 때와 같은 전진폭으로 측정되고, 글꼴 대체는 글자 단위로 동작합니다. 그래서 라틴·그리스·키릴·CJK가 한 문자열에 섞여도 서체를 고를 필요가 없습니다. 입력기는 입력란 안에서 조합하고, 편집은 자소 클러스터 단위로 움직여 결합 기호나 여러 부분으로 된 이모지가 잘리지 않습니다. 오른쪽에서 왼쪽은 셰이핑만이 아닙니다. 방향은 서브트리가 상속하는 축이라 히브리어 창은 거울처럼 뒤집히고, 그 안에 왼쪽에서 오른쪽으로 읽는 코드 패널을 둘 수도 있습니다.",
  "home.languages.alt":
  ...
  "home.limits.scripts.heading": "가로쓰기만",
  "home.limits.scripts.body":
    "아랍 문자, 히브리 문자, 데바나가리, 타이 문자는 텍스트가 그려지는 곳이라면 어디서나 셰이핑되고, 배치는 상속되는 오른쪽에서 왼쪽 축으로 뒤집히며, 숫자와 정렬과 대소문자는 언어를 따릅니다. ar과 he 번역을 배포하고 문자 체계마다 볼드도 있습니다. 없는 것은 세로쓰기입니다. 위에서 아래로 흐르는 단도, 배치를 관통하는 두 번째 축도 없습니다. 줄은 접히지만 양쪽 정렬은 하지 않습니다.",
  "home.limits.a11y.heading": "화면 낭독기, 그리고 그 한계",
  "home.limits.a11y.body":
    "창은 UI Automation과 NSAccessibility와 AT-SPI2에 자신을 공개하고, NVDA와 VoiceOver와 Orca가 Limn 인터페이스를 그것이 그려진 언어로 읽습니다. 텍스트는 통째로 읽고 쓰기 때문에 글자 단위 검토와 점자 커서 라우팅은 제한됩니다. 목록은 실체화된 행만 공개합니다. 그리고 고대비, 동작 줄이기, 글자 크기 같은 운영체제의 접근성 설정은 전혀 노출하지 않습니다.",
  ...
  "showcase.description":
    "툴킷이 그린 화면 전체: 데모 애플리케이션, 오른쪽에서 왼쪽으로 뒤집힌 같은 창, 3D 뷰포트, 폼, 배치한 창.",
  "showcase.rtl.heading": "같은 창을, 오른쪽에서 왼쪽으로",
  "showcase.rtl.body":
    "ar 번역에 방향 축을 뒤집은 데모 애플리케이션: 메뉴 막대, 탭, 입력란, 스크롤 막대, 성능 푸터가 모두 오른쪽에서 읽히고, 그 안의 텍스트는 아랍 문자답게 결합하고 재배열됩니다. 화면을 다시 쓰지 않았습니다. 방향은 컨트롤 크기처럼 설정하는, 상속되는 하나의 축입니다.",
  ...
  "privacy.description":
    "이 사이트가 저장하는 것과 저장하지 않는 것, 그리고 선택을 바꾸는 방법. 측정을 허용하기 전에는 쿠키도 제3자 요청도 없으며, 측정은 꺼진 채로 도착합니다.",
  "privacy.eyebrow": "개인정보",
  "privacy.heading": "이 사이트가 저장하는 것",
  "privacy.lede":
    "짧게 말하면, 측정이 꺼져 있는 동안에는 — 처음에는 꺼져 있습니다 — 쿠키도, 제3자 요청도, 당신을 특정하는 것도 없습니다. 허용해야만 이 사이트가 Google Analytics를 불러옵니다. 긴 설명은 아래에 있습니다. 짧은 설명은 긴 설명이 같은 말을 할 때에만 읽을 가치가 있으니까요.",
  ...
  "privacy.cookies.heading": "쿠키는 측정을 허용할 때만",
  "privacy.cookies.body":
    "사이트 자체는 어떤 쿠키도 설정하지 않습니다. 측정이 꺼져 있는 동안에는 요청에 무언가가 붙지도, 다른 사이트까지 따라가지도 않습니다. 허용하면 Google Analytics가 자체 쿠키(<code>_ga</code>, <code>_ga_…</code>)를 설정합니다. 로컬 저장소는 쿠키가 아닙니다. 전송되지 않으며 서버가 요구할 수도 없습니다.",
  "privacy.analytics.heading": "측정은 허용하기 전까지 꺼져 있습니다",
  "privacy.analytics.body":
    "이 사이트는 Google Analytics를 사용하며, 오직 허용한 경우에만 그렇습니다. 개인정보 안내의 측정 스위치는 기본이 꺼짐이고, 그 꺼짐은 약속이 아니라 구조로 강제됩니다. 태그는 어떤 브라우저도 실행하지 않는 <code>text/plain</code> 블록으로 배포되며, 허용한 순간에야 실행되는 스크립트로 바뀝니다. 그 전에는 아닙니다. 허용을 철회하면 다시는 불러오지 않습니다.",
  "privacy.thirdParty.heading": "측정을 허용하기 전까지, 바깥에서 불러오는 것 없음",
  "privacy.thirdParty.body":
    "글꼴, 이미지, 스타일시트, 스크립트가 모두 이 도메인에서 옵니다. 웹폰트 서비스도 CDN도, 삽입된 영상도 소셜 위젯도 없습니다. 측정이 꺼져 있으면 여기서 페이지를 읽을 때 정확히 한 대의 서버와만 통신합니다. 허용하면 <code>googletagmanager.com</code>에서 태그를 추가로 내려받습니다.",
