...
export const ko: Catalog = {
  ...
  "home.languages.body":
    "라틴·그리스·키릴·CJK 문자가 한 문자열에 섞이고, 글꼴은 같은 문자 체계의 구간마다 선택됩니다. 아랍 문자, 히브리 문자, 데바나가리, 태국 문자는 올바르게 셰이핑되고(연결형, 합자, 결합 기호), 오른쪽에서 왼쪽으로 쓰는 언어에서는 창 전체가 거울상이 됩니다: 행, 여백, 스크롤바, 화살표 키. 입력기는 필드 안에서 조합하고, 편집은 자소 단위로 움직입니다.",
  "home.languages.alt":
  ...
  "showcase.editor.heading": "파일을 쓰는 프로그램으로서의 테마 편집기",
  "showcase.editor.body":
    "팔레트는 데이터이므로 그것을 편집하는 일은 하나의 화면이 됩니다. 그리고 이것은 당신의 애플리케이션이 짊어지는 의존성이 아니라, 실행해서 쓰는 프로그램입니다. 저장되는 것은 테마 파일이고, 애플리케이션은 limn-toolkit만으로 ThemeFormat을 써서 읽어 들입니다. 모서리 슬라이더를 끌면 같은 프레임 안에서 창이 옷을 갈아입습니다. 화면 속 모든 입력란과 버튼과 색 견본이 함께. 옆의 보고서는 각 잉크를 그것이 놓일 수 있는 모든 표면에 대해 재므로, 명도 대비가 부족한 팔레트는 눈에 보이게 떨어집니다.",
