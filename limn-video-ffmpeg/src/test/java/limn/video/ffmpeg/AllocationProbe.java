...
/**
 * Measures what a stretch of work allocates on the calling thread, for the claim that a
 * steady-state decode allocates nothing at all.
 *
 * <p>The counter is exact rather than sampled: it reports the bytes the thread requested, not what
 * survived a collection, so a single {@code new} inside a measured loop shows up. The work is
 * passed as a {@link Runnable} whose capture happens at the call site, before measuring starts, so
 * building it costs the measurement nothing.
 */
final class AllocationProbe {
     ...
     *         nothing look as though it does; work that genuinely allocates does so every time.
     */
