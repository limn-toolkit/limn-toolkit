 ...
 * Which set they count is each widget's to say, and three say it differently: a list row, a tab,
 * a menu row or a radio counts the container's members; a tree row counts its
 * <em>siblings</em>, the rows at its own level under its own parent, its place in the whole
 * outline being {@link HierarchyFacet}'s; a calendar day counts the days of its own month, "15 of
 * 30". A bridge that needs a row's index in a grid reads {@link CellFacet}, never these numbers.
 *
 * <p>A zero says there is no number: a bridge publishes nothing for it on every platform rather
 * than "0 of 0", which is what a radio outside any group carries.
 *
 ...
 * ancestors that lack one; it is what a selection change is raised on and what a platform's
 * "selection container" answers. A radio button has none — a {@code ButtonGroup} is not a node —
 * and says so with {@link #containerless()}, so its selection change is never laid on whatever
 * layout node happens to be its published parent, and a bridge asked for its container answers none
