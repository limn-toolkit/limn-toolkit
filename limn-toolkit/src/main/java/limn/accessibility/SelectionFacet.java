 ...
 * It is the tree's: {@link AccessibleTree#activeDescendant()} is the first node published
 * {@link Accessible.State#ACTIVE} below the focused node, whichever containers lie between, and one
 * event names the focused node when it moves. Until 2026-09-14 this record carried an
 * {@code activeDescendant} resolved per container, so a combo's layer and its list, or a menu
 * surface and each of its columns, announced one arrow key two or three times over, and a container
 * nobody was in announced a cursor nobody had.
 *
